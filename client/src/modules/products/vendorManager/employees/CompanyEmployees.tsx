...
export default function CompanyEmployees() {
  ...
  const filteredEmployees = useMemo(() => employees.filter((employee) =>
    [employee.name, employee.email, employee.phone, employee.department, employee.role, employee.reporting_manager]
      .some((value) => searchable(value).includes(query))), [employees, query]);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
        <div className="flex justify-end border-b border-gray-100 p-4"><label className="relative block w-full sm:w-80"><FiSearch className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search employees..." className="w-full rounded-xl border border-gray-200 bg-gray-50 py-2.5 pl-10 pr-4 text-sm outline-none focus:border-[#852BAF] focus:bg-white focus:ring-4 focus:ring-purple-100" /></label></div>
