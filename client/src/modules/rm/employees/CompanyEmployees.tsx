...
export default function CompanyEmployees() {
  ...
  const [statusTab, setStatusTab] = useState<"activated" | "not-activated">("activated");
  ...
  const filteredEmployees = useMemo(() => employees.filter((employee) => {
    const matchesStatus = statusTab === "activated"
  ...
  const activatedCount = employees.filter((employee) => employee.customer_id !== null).length;
  const notActivatedCount = employees.length - activatedCount;
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-gray-100 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="inline-flex w-fit rounded-xl bg-gray-100 p-1"><button onClick={() => setStatusTab("activated")} className={`rounded-lg px-4 py-2 text-sm font-bold transition ${statusTab === "activated" ? "bg-white text-emerald-700 shadow-sm" : "text-gray-500 hover:text-gray-800"}`}>Activated Employees ({activatedCount})</button><button onClick={() => setStatusTab("not-activated")} className={`rounded-lg px-4 py-2 text-sm font-bold transition ${statusTab === "not-activated" ? "bg-white text-amber-700 shadow-sm" : "text-gray-500 hover:text-gray-800"}`}>Not Activated ({notActivatedCount})</button></div><label className="relative block w-full sm:w-80"><FiSearch className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search employees..." className="w-full rounded-xl border border-gray-200 bg-gray-50 py-2.5 pl-10 pr-4 text-sm outline-none focus:border-[#852BAF] focus:bg-white focus:ring-4 focus:ring-purple-100" /></label></div>
        {error ? <p className="p-12 text-center text-sm font-semibold text-red-600">{error}</p> : <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-gray-50 text-xs uppercase text-gray-500"><tr><th className="px-5 py-3">Employee</th><th className="px-5 py-3">Department / Role</th><th className="px-5 py-3">Joining</th><th className="px-5 py-3">Reporting manager</th><th className="px-5 py-3">Platform</th><th className="px-5 py-3">Company status</th><th className="px-5 py-3">Account</th></tr></thead><tbody className="divide-y divide-gray-100">
          {filteredEmployees.map((employee) => <tr key={employee.id} className="hover:bg-purple-50/30"><td className="px-5 py-4"><p className="font-bold text-gray-900">{employee.name}</p><p className="text-xs text-gray-500">{employee.email || "No email"}</p><p className="text-xs text-gray-400">{employee.phone || "No phone"}</p></td><td className="px-5 py-4"><p className="font-semibold text-gray-700">{employee.department || "Unassigned"}</p><p className="text-xs text-gray-400">{employee.role || "No role"}</p></td><td className="px-5 py-4 text-gray-600">{displayDate(employee.date_of_joining)}</td><td className="px-5 py-4 text-gray-600">{employee.reporting_manager || "—"}</td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-[11px] font-extrabold uppercase ${employee.device_platform === "android" ? "bg-emerald-50 text-emerald-700" : employee.device_platform === "ios" ? "bg-slate-900 text-white" : "bg-gray-100 text-gray-500"}`}>{employee.device_platform === "android" ? "Android" : employee.device_platform === "ios" ? "iOS" : "Unknown"}</span>{employee.device_name && <p className="mt-1 max-w-32 truncate text-[11px] text-gray-400" title={employee.device_name}>{employee.device_name}</p>}</td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-[11px] font-bold uppercase ${Number(employee.status) === 1 ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>{Number(employee.status) === 1 ? "Active" : "Inactive"}</span></td><td className="px-5 py-4"><p className={`text-xs font-bold ${employee.customer_id ? "text-emerald-600" : "text-amber-600"}`}>{employee.customer_id ? "Activated" : "Not activated"}</p>{employee.customer_id && <p className="mt-1 text-[11px] text-gray-400">{Number(employee.customer_is_verified) === 1 ? "Verified" : "Not verified"}</p>}</td></tr>)}
