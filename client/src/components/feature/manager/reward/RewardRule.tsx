  ...
  return (
    <div className="w-full min-h-screen">
      <div className="p-6 bg-white border border-gray-200 shadow-lg rounded-2xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">Manage reward configurations</p>
            </div>
          </div>
        ...
        {/* ERROR */}
        {error && (
          <p className="px-4 py-3 mb-4 text-center text-red-600 border border-red-200 bg-red-50 rounded-xl">{error}</p>
        )}
          ...
          <div className="overflow-x-auto border border-gray-100 rounded-2xl">
            <table className="w-full text-sm text-left">
              <thead style={{ background: "linear-gradient(135deg, #fdf8ff 0%, #fff5f8 100%)" }}>
                <tr>
                  {["Name","Type","Value","Max Reward","Min Order","Max Order","Priority","Stackable","Expiry","Validity","Source","Status","Action"].map((h) => (
                    <th key={h} className="px-4 py-4 text-xs font-bold tracking-wider text-gray-500 uppercase whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-gray-50">
                {rules.length === 0 ? (
                  <tr>
                    <td colSpan={13} className="py-16 text-sm text-center text-gray-400">
                      No reward rules found
                    </td>
                  </tr>
                ) : (
                  rules.map((rule) => (
                    <tr key={rule.reward_rule_id} className="transition-colors bg-white hover:bg-purple-50/30">
                      <td className="px-4 py-3 font-semibold text-gray-800">{rule.name}</td>

                      <td className="px-4 py-3 text-gray-600 capitalize">{rule.reward_type}</td>

                      <td className="px-4 py-3 font-medium text-gray-800">
                        {rule.reward_type === "percentage" ? `${rule.reward_value}%` : `₹${rule.reward_value}`}
                      </td>

                      <td className="px-4 py-3 text-gray-600">{rule.max_reward ?? "-"}</td>

                      <td className="px-4 py-3 text-gray-600">₹{rule.min_order_amount}</td>

                      <td className="px-4 py-3 text-gray-600">
                        {rule.max_order_amount ? `₹${rule.max_order_amount}` : "-"}
                      </td>

                      <td className="px-4 py-3 text-gray-600">{rule.priority}</td>

                      <td className="px-4 py-3 text-gray-600">{rule.is_stackable ? "Yes" : "No"}</td>

                      <td className="px-4 py-3 text-gray-600">
                        {rule.expiry_days ? `${rule.expiry_days} days` : "No expiry"}
                      </td>

                      <td className="px-4 py-3 text-xs whitespace-nowrap">
                        {!rule.start_date && !rule.end_date && <span className="text-gray-400">Always Active</span>}
                        {rule.start_date && !rule.end_date && <span className="text-[#852BAF]">Starts: {formatDate(rule.start_date)}</span>}
                        {!rule.start_date && rule.end_date && <span className="text-orange-600">Until: {formatDate(rule.end_date)}</span>}
                        {rule.start_date && rule.end_date && (
                          <span className="text-[#852BAF]">{formatDate(rule.start_date)} → {formatDate(rule.end_date)}</span>
                        )}
                      </td>

                      <td className="px-4 py-3 text-gray-600">{rule.source_type}</td>
                      ...
                      <td className="px-4 py-3">
                        {(() => {
                          ...
                          return (
                            <span className={`inline-flex px-2.5 py-0.5 rounded-full border text-[11px] font-semibold uppercase tracking-wide ${colorMap[status.color] ?? colorMap.gray}`}>
                              {status.label}
                            </span>
                          );
                        })()}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <button
                            onClick={() => navigate(`/manager/reward-edit/${rule.reward_rule_id}`)}
