...
class ModuleIconModel {
  ...
  async getActiveModules() {
    const [rows] = await db.query(
      `
      SELECT icon_id, module_key, icon_type, icon_url, active_icon_url, route_key, label, sort_order, is_active
      FROM module_icons
      WHERE is_active = 1
  ...
  async getAllModules() {
    const [rows] = await db.query(
      `
      SELECT icon_id, module_key, icon_type, icon_url, active_icon_url, route_key, label, sort_order, is_active, created_by_name, created_at, updated_at
      FROM module_icons
      ORDER BY sort_order ASC
      `,
    );
  ...
  async createModule(data) {
    ...
    try {
      await db.query(
        `
        INSERT INTO module_icons (module_key, icon_type, icon_url, active_icon_url, label, sort_order, is_active, created_by_name)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `,
        [
          data.module_key,
  ...
  async updateModule(moduleKey, data) {
    ...
    const settable = ["icon_type", "icon_url", "active_icon_url", "label", "sort_order", "is_active", "created_by_name"];
